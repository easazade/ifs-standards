...
Parse `$ARGUMENTS` as: first word = entity name, remainder = entity specification.

Create a new IFS entity for that name, or update it if it already exists.
...
Do not create or update files immediately. First interview the user in one batch so they can answer all questions at once.
...
- Parse the user's answers when they reply.
...
Run this only after schema files have been written. Do not run it during the first questionnaire response.
